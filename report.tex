...
\begin{figure}[ht] 
  \centering 
\subfigure[Singular values for the binary connection matrix.]{\includegraphics[width=0.49\linewidth]{singular_values.png} \label{fig:sing_va}}
\subfigure[Influences of connections.]{\includegraphics[width=0.49\linewidth]{connections_influence.png}\label{fig:influence}}
...
Graph metrics are scalar functions of the weight matrix $X$ of a graph 
\begin{equation}
f_i(\bm{X})=K_i
\label{eq:metrics}
\end{equation}

They quantify a property of the network. In this work, the metrics involve time delays ($\delta_{ij}$) or the physical distances ($r$) between stations. The connection strength of a node $i$ that is the row sum of the normalized weighted adjacency matrix: 
\begin{align}
...
K_i &= f_i (\bm{W})= \sum_j{w_{ij}}  \quad \text{for }  \mathcal{G}_{\delta} = (\mathcal{V}, \mathcal{E}, \bf{W}).
\end{align}
