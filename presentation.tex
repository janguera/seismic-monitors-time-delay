...
\item Weighted normalized graphs $\mathcal{G}_{\delta} = (\mathcal{V}, \mathcal{E}, \bm{\hat{\delta}})$, $\mathcal{G}_r = (\mathcal{V}, \mathcal{E}, \bm{W})$
%\item $w_{ij}=f(r_{ij})$
%\item Values $\hat{\delta}_{ij}$,  $w_{ij}$ are normalized to $[0,1]$
\item Metric:
\begin{equation}
...
\item A normalized distance measure was selected as graph metrics $K_m$
\begin{equation}
...
\begin{tikzpicture}[main node/.style={circle,draw,font=\sffamily\Large\bfseries},>=stealth',shorten >=1pt,auto,node distance=2.8cm,semithick, scale = 0.5]
=======
\begin{figure}[htb]
\centering
\begin{tikzpicture}[main node/.style={circle,draw,font=\sffamily\Large\bfseries},>=stealth',shorten >=1pt,auto,node distance=2.8cm,
                    semithick]
>>>>>>> 0cc94ad65fc3cbaef419c266beaa1767da074aaa
...
\begin{frame}
\frametitle{Extracting clock drift from noise}
\begin{itemize}
